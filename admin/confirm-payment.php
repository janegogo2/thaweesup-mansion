...
$filterDefault = " where status.status_id='" . ACTIVE_CONTRACT . "' and stay_status in (" . STAY_STATUS . "," . REQUEST_CHECK_OUT_STATUS . ")
and contract_no in (
select contract_no from payment_room where YEAR(save_date) = '$year' and MONTH(save_date) = '$month'
and status_id in ('" . WAITING_PAYMENT . "','" . PAYMENT_SUCCESS . "')
) ";
                    ...
                    <form class="form-horizontal" id="form_data" name="form_data" enctype="multipart/form-data"
                        ...
                        <div class="">
                            ...
                            <div class="col-sm-12">
                                <div class="box box-primary">
                                    <div class="box-header with-border">
                                        <h3 class="box-title">เก็บค่าเช่า</h3>
                                    </div>
                                            ...
                                            <div style="overflow-x: auto" class="col-sm-12">
                                                <table class="table table-hover tbgray" id="tbView">
                                                    <tr>
                                                        ...
                                                        <th width="10%">จ่ายเงิน</th>
                                                    </tr>
                                                    <tbody>
                                                    <?php
                                                    $index = $for_start;
                                                    foreach ($select_all as $row) {
                                                        $payment = $conn->queryRaw("select * from payment_room where YEAR(save_date) = '$year' and MONTH(save_date) = '$month'
                                                            and contract_no = '" . $row['contract_no'] . "'", true);
                                                        $index++;
                                                        ?>
                                                        <tr>
                                                            ...
                                                            <td class="active text-center"
                                                                nowrap><?php echo $payment['status_id'] == WAITING_PAYMENT ? "รอชำระเงิน" : "ชำระเงินแล้ว"; ?></td>
                                                            <td class="active" nowrap align="center">
                                                                <div class="btn-group">

                                                                    <?php if ($payment['status_id'] == WAITING_PAYMENT) { ?>
                                                                        <a class="btn btn-warning btn-xs"
                                                                           href="confirm-payment-update.php?__contract_no=<?php echo $row['contract_no']; ?>&__year=<?php echo $year; ?>&__month=<?php echo $month; ?>"><i
                                                                                class="fa fa-edit"></i> </a>
                                                                    <?php } else { ?>
                                                                        <a class="btn btn-info btn-xs" target="_blank"
                                                                           href="confirm-payment-print.php?__payment_no=<?php echo $payment['payment_no']; ?>&__year=<?php echo $year; ?>&__month=<?php echo $month; ?>"><i
...
<!-- Page script -->
<script>
    $('#menu-confirm-payment,#menu-payment').addClass('active');
