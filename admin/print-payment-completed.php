...
$sql = "select payment_room.*,status_name,room_name,contract_room.contract_no as contract_no
,concat(customer.first_name,' ',customer.last_name) as customer_name
from payment_room
left join contract_room on contract_room.contract_no=payment_room.contract_no
left join status on status.status_id=payment_room.status_id
...
left join room on room.room_id=contract_room.room_id where  payment_room.status_id = '" . PAYMENT_SUCCESS . "'
and MONTH(save_date)>='$month' and MONTH(save_date)<='$month2'
and contract_room.customer_id='$name' or contract_room.room_id='$room'  and  payment_room.status_id = '" . PAYMENT_SUCCESS . "'
 or $name=0 and $room=0  and  payment_room.status_id = '" . PAYMENT_SUCCESS . "'";
...
?>
    ...
    <div style="text-align: center;">รายงานการชำระเงิน</div>
    <br>
    ...
    font-size: 10px;
    background-color: white;
    text-align: right;"><?php echo $row['mulct_price']; ?></td>
                <td style="padding: 7px;
    vertical-align: top;
    font-size: 10px;
    background-color: white;
    text-align: right;"><?php echo $row['total_price']; ?></td>
            </tr>
        <?php } ?>
        </tbody>
    </table>
